...
const CategoryWiseProduct = ({ category, heading }) => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const categoryProduct = await axios.post(
        `${process.env.REACT_APP_API_URL}/getCategoryWiseProduct`,
        ...
        {loading ? (
          <div className="mt-6 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
            {loadingList.map((_, index) => (
              ...
              >
                <div className="bg-slate-200  w-full overflow-hidden rounded-md aspect-none group-hover:opacity-75 h-72 animate-pulse"></div>
