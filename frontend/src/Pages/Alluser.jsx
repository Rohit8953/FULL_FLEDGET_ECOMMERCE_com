...
const Alluser = () => {
    ...
    const fetchAllUsers = async() =>{
        const dataResponse = await axios.get(`${process.env.REACT_APP_API_URL}/allusers`,{
                    ...
                    allUser.map((el,index) => {
                        return(
                            <tr>
                                ...
                                <td className='border-[1px] border-black text-center'>{el?.name}</td>
                                <td className='border-[1px] border-black text-center'>{el?.email}</td>
                                <td className='border-[1px] border-black text-center'>{el?.role}</td>
                                <td className='border-[1px] border-black text-center'>{moment(el?.createdAt).format('LL')}</td>
